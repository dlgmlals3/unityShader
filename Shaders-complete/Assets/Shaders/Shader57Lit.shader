Shader "NiksShaders/Shader57Lit"
{
    ...
    half4 LightingPhong (SurfaceOutput s, half3 lightDir, half viewDir, half atten){
		...
		half3 specularity = pow(RdotV, _Glossiness*20)*_SpecPower *_SpecColor.rgb ;

		half4 c;
		c.rgb = (s.Albedo * _LightColor0.rgb * NdotL + _LightColor0.rgb * specularity) * atten;
		c.a = s.Alpha;
		return c;
	}
	...
	half4 LightingMyBlinnPhong (SurfaceOutput s, half3 lightDir, half viewDir, half atten){
		...
		//Specular calculations
		half3 specularity = pow(NdotV, _Glossiness*48)*_SpecPower *_SpecColor.rgb ;
